...
*/
bool VerifyFirmware(hid_device *handle, bool opt_quiet)
{
	BLCOMMAND_t cmd;
	cmd.report_id = 0;
...
*/
bool GetBootloaderInfo(hid_device *handle, uint8_t *target_mcu_id, uint8_t *target_mcu_fuses)
{
	BLCOMMAND_t cmd = { .report_id = 0, .params.u32 = 0 };
	uint8_t buffer[BUFFER_SIZE];
...
/**************************************************************************************************
* Update firmware via HID
*/
int HID_mode(hid_device *handle)
{
	...
	// get bootloader info
	if (!GetBootloaderInfo(handle, target_mcu_id, target_mcu_fuses))
		return 1;
	...
	if (opt_verify && (!VerifyFirmware(handle, opt_quiet)))
		return 1;
