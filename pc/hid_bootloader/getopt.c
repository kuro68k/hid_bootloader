/* Getopt for GNU. NOTE: getopt is now part of the C library, so if you
   don't know what "Keep this file name-space clean" means, talk to
   drepper@gnu.org before changing it! Copyright (C)
...
/* Comment out all this code if we are using the GNU C Library, and are
   not actually compiling the library itself. This code is part of the GNU 
...
/* Describe how to deal with options that follow non-option ARGV-elements.
   ...
   variable POSIXLY_CORRECT, or using `+' as the first character of the
...
static enum { REQUIRE_ORDER, PERMUTE, RETURN_IN_ORDER
} ordering;
...
/* If using GCC, we can safely declare strlen this way. If not using GCC,
   it is ok not to declare it. */
#ifdef __GNUC__
/* Note that Motorola Delta 68k R3V7 comes with GCC but not stddef.h. That 
   was relevant to code that was here before. */
# if (!defined __STDC__ || !__STDC__) && !defined strlen
/* gcc with -traditional declares the built-in strlen to return int, and
   has done so at least since version 2.4.5. -- rms. */
...
#ifdef _LIBC
/* Stored original parameters. XXX This is no good solution. We should
   rather copy the args so that we can compare them later. But we must not 
   use malloc(3). */
extern int __libc_argc;
extern char **__libc_argv;

/* Bash 2.0 gives us an environment variable containing flags indicating
   ARGV elements that should not be considered arguments. */
...

{
	...
	/* First make sure the handling of the `__getopt_nonoption_flags'
	   string can work normally. Our top argument must be in the range of
	   the string. */
...
/* Scan elements of ARGV (whose length is ARGC) for option characters
   ...
   an option character is seen that is not listed in OPTSTRING, return '?' 
   ...
   option name in the same ARGV-element, separated from the option name by 
   a `=', or else the in next ARGV-element. When `getopt' finds a
   long-named option, it returns 0 if that option's `flag' field is
   nonzero, the value of the option's `val' field if the `flag' field is
   zero.
   ...
   is only valid when a long-named option has been found by the most
   recent call.
...

{
	...
	/* Test whether ARGV[optind] points to a non-option argument. Either
	   it does not have option syntax, or there is an environment flag
	   from the shell indicating it is not an option. The later
	   information is only used when the used in the GNU libc. */
#if defined _LIBC && defined USE_NONOPTION_FLAGS
# define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0' \
	||(optind <
	nonoption_flags_len \ &&__getopt_nonoption_flags[optind] == '1'))
#else							/* */
# define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0')
#endif							/* */
		...
		if (pfound != NULL) {
			...
			if (*nameend) {
				...
				/* Don't test has_arg with >, because some C compilers
				   don't allow it to be used on enums. */
				if (pfound->has_arg)
					optarg = nameend + 1;
...
%s: option `%c%s' doesn't allow an argument\n"), argv[0], argv[optind - 1][0], pfound->name);
		...
		/* Can't find it as a long option. If this is not
		   ...
		   valid short option, then it's an error. Otherwise interpret it
		   as a short option. */
	...
	/* Look at and handle the next short option-character. */
	{
		...
		if (temp[0] == 'W' && temp[1] == ';') {
			...
			/* optarg is now the argument, see if it's in the table of
			   longopts. */
